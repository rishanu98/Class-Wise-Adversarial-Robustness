...
class LinfPGDAttack(object):
    ...
    def pgd_linf_targeted(self,model, x_natural, y_targ, epsilon, alpha, k, device):
        x = x_natural.detach()
        x = x + torch.zeros_like(x).uniform_(-epsilon, epsilon)
        for i in range(k):
            x.requires_grad_()
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(train_loader):
        ...
        if batch_idx % 10 == 0:
            ...
            print('Current adversarial Untargeted train accuracy:', str(predicted.eq(targets).sum().item() / targets.size(0)))
            print('Current adversarial train loss:', loss.item())

    print('\nTotal adversarial Untargeted train accuarcy:', 100. * correct / total)
...
def test(epoch):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(test_loader):
            ...
            #Targeted train Accuracy
            adv = adversary.pgd_linf_targeted(net, inputs, targets, epsilon, alpha, k, device)
    ...
    print('Total adversarial test loss:', Targ_loss)

    # Plot the targeted attack confusion matrix
    ...
    plt.ylabel('True')
    plt.title('Confusion Matrix for UnTargeted Training and Targeted Attack Testing')
    plt.savefig('Confusion Matrix for UnTargeted Training and Targeted Attack Testing')
    plt.show()
